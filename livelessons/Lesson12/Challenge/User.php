...
class User
{
     ...
     */
    public static function registerUser($un, $fn, $pw, $em, $bd, $gen,
                                        $priv = User::PRIVILEGES_REGULAR_USER)
    {
        if ($un == '' or $fn == '' or $pw == '' or $em == '' or $bd == '' or $gen == '')
            throw new PSInvalidUserDataException();

        $pw = md5($pw);

        $ud = UserData::fetch();
        $uid = $ud->createUser($un, $fn, $pw, $em, $bd, $gen, User::PRIVILEGES_REGULAR_USER);

        return new User($uid, $un, $fn, $pw, $em, $bd, $gen, User::PRIVILEGES_REGULAR_USER);
    }



    public function __construct($userid, $username, $fullname, $password,
                                $email_address, $birthdate, $gender,
