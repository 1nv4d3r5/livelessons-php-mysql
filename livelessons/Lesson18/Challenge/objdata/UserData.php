...
EOQ;
    ...
    public function searchUserBios($searchstr, $start, $cusers)
    {
        if ($start < 0 or $cusers < 1)
            throw new PSInvalidArgumentException();

        $ss = $this->escapeString($searchstr);

        $query = <<<EOQ
SELECT SQL_CALC_FOUND_ROWS Users.*, UserBios.userbio
  FROM Users, UserBios
 WHERE Users.id = UserBios.userid
       AND Match (UserBios.userbio) AGAINST('$ss')
 LIMIT $start, $cusers
EOQ;
        $conn = $this->getConnection();
        $results = $conn->query($query);
        ...
        // now get the count.
        $results = $conn->query('SELECT FOUND_ROWS()');
        $fr = $results->fetch_array();
        $results->close();
        $count = $fr[0];

        return array('users' => $output, 'count' => $count);
    }
