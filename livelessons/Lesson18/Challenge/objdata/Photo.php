...
class Photo
{
    ...
    public function __construct($pid, $fn, $uid, $up, $desc)
    {
    ...
    public static function addNewPhotoForUser($userid, $filename, $desc)
    {
        if ((int)$userid <= 0 or $filename == '')
            throw new PSInvalidPhotoDataException();

        $row = PhotoData::fetch()->addNewPhotoForUser($userid, $filename, $desc);
    ...
    {
        return new Photo($row['id'], $row['filename'], $row['userid'], $row['uploaded'], $row['photodesc']);
    }
